...
class Projector {
    ...
    _renderData.objects.forEach((o) {
      ...
      if (object is Mesh) {
        ...
        vertices.forEach((Vector3 v) {
          ...
          _vertex.positionScreen = new Vector4(_vertex.positionWorld.x, _vertex.positionWorld.y, _vertex.positionWorld.z, 1.0); //  _vertex.positionWorld.clone();
        ...
        for (f = 0; f < fl; f++) {
          ...
          if (allVtxVisible) {
            if (face.size == 3) {
              visible = (((vtx[2].positionScreen.x - vtx[0].positionScreen.x) * (vtx[1].positionScreen.y - vtx[0].positionScreen.y) - (vtx[2].positionScreen.y - vtx[0].positionScreen.y) * (vtx[1].positionScreen.x - vtx[0].positionScreen.x)) < 0);
            } else if (face.size == 4) {
              visible = (vtx[3].positionScreen.x - vtx[0].positionScreen.x) * (vtx[1].positionScreen.y - vtx[0].positionScreen.y) - (vtx[3].positionScreen.y - vtx[0].positionScreen.y) * (vtx[1].positionScreen.x - vtx[0].positionScreen.x) < 0 || (vtx[1].positionScreen.x - vtx[2].positionScreen.x) * (vtx[3].positionScreen.y - vtx[2].positionScreen.y) - (vtx[1].positionScreen.y - vtx[2].positionScreen.y) * (vtx[3].positionScreen.x - vtx[2].positionScreen.x) < 0;
            }
  ...
  bool clipLine(Vector4 s1, Vector4 s2) {
    double alpha1 = 0.0;
    double bc2far = -s2.z + s2.w;
    double bc1far = -s1.z + s1.w;
    double bc2near = s2.z + s2.w;
    double // Calculate the boundary coordinate of each vertex for the near and far clip planes,

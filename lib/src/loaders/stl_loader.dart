...
class STLLoader extends Loader {
  ...
  Future<Geometry> load(url) => HttpRequest.request(url, responseType: "arraybuffer").then((req) => _parse(req.response));
  ...
  _parseBinary(ByteBuffer bytes) {
    ...
    for (var face = 0; face < n_faces; face++) {
      ...
      var normal = new Vector3(data.getFloat32(start, Endianness.LITTLE_ENDIAN), data.getFloat32(start + 4, Endianness.LITTLE_ENDIAN), data.getFloat32(start + 8, Endianness.LITTLE_ENDIAN));

      for (var i = 1; i <= 3; i++) {
        var vertexstart = start + i * 12;
        geometry.vertices.add(new Vector3(data.getFloat32(vertexstart, Endianness.LITTLE_ENDIAN), data.getFloat32(vertexstart + 4, Endianness.LITTLE_ENDIAN), data.getFloat32(vertexstart + 8, Endianness.LITTLE_ENDIAN)));
      }
